...
class MusicBot(discord.Client):
    ...
    async def cmd_stream(self, channel):
        ...
        querystring = {"channel":"41719107,23161357,26490481"}

        headers = {
            'accept': "application/vnd.twitchtv.v5+json",
            'client-id': "c7wux8jp0da4xv6w82qbi8ai9z203m",
            'cache-control': "no-cache"
            }
        response = requests.request("GET", url, headers=headers, params=querystring)
        streamers = {x['channel']['url'] for x in response.json().get('streams', [])}
        if streamers:
            await self.safe_send_message(channel, "Streaming:\n" + '\n'.join(streamers))
        else:
            await self.safe_send_message(channel, "No one")
    ...
    async def cmd_add(self, player, channel, server, message):
        """
        Usage:
            {command_prefix}add
